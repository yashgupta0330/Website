...
@Component({
  ...
  templateUrl: './interactive-demos.component.html',
...
export class InteractiveDemosComponent implements OnInit {
  ...
  private tryScrollToCardFromSession(): void {
    ...
    try {
      ...
      if (source === '/solutions/interactive-demos') {
        cardId = cid;
      }
    } catch {}
    if (!cardId) return;
  ...
  onCaseStudyClick(cardId: string): void {
    ...
    this.scrollRestoration.saveScrollPosition('/solutions/interactive-demos');
    // Save navigation context for solution-aware back navigation
    sessionStorage.setItem(
      'solution_case_study_nav',
      JSON.stringify({ source: '/solutions/interactive-demos', cardId })
    );
  ...
  loadFaqs(): void {
    const faqFilePath = '/assets/data/solutions/interactive-demos.json';
